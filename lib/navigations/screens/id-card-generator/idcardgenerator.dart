...
import 'package:intl/intl.dart'; // 🔥 For formatting DOB only date
...
class _IDCardGeneratorState extends State<IDCardGenerator> {
  ...
  void initState() {
    ...
    // 🔥 Auto-fill check whenever roll changes
    rollController.addListener(_checkAndAutofill);
    nameController.addListener(_checkAndAutofill);
  }
  ...
  Future<void> _checkAndAutofill() async {
    ...
    try {
      // Assume studentId is known: "students/{studentId}"
      // We query Firestore collection with roll & name match
      final query =
